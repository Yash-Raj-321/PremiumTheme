...
if(class_exists('WP_Customize_Control')){
    ...
    class Ruh_Switch_Control extends WP_Customize_Control{
        public $type = 'switch';
    ...
    class Ruh_Customize_Heading extends WP_Customize_Control {
        public $type = 'heading';
...
class Ruh_Fontawesome_Icon_Chooser extends WP_Customize_Control{
    public $type = 'icon';
    ...
    public function render_content(){
        ...
        <label>
            ...
            <ul class="total-icon-list clearfix">
                <?php
                $ruh_font_awesome_icon_array = ruh_font_awesome_icon_array();
                foreach ($ruh_font_awesome_icon_array as $ruh_font_awesome_icon) {
                    $icon_class = $this->value() == $ruh_font_awesome_icon ? 'icon-active' : '';
                    echo '<li class='.$icon_class.'><i class="'.$ruh_font_awesome_icon.'"></i></li>';
                }
                ?>
            </ul>
...
class ruh_Dropdown_Chooser extends WP_Customize_Control{
...
class ruh_Display_Gallery_Control extends WP_Customize_Control{
    public $type = 'gallery';
...
class Ruh_Customize_Checkbox_Multiple extends WP_Customize_Control {
    ...
    class ruh_Dropdown_Multiple_Chooser extends WP_Customize_Control{
    ...
    class ruh_Category_Dropdown extends WP_Customize_Control{
        private $cats = false;
...
class ruh_Info_Text extends WP_Customize_Control{
    public function render_content(){
        ?>
        <span class="customize-control-title">
            <?php echo esc_html( $this->label ); ?>
...
class ruh_Font_Chooser extends WP_Customize_Control{
    public $type = 'dropdown_chooser';
 ...
 **/function backgroundManager($wp_customize, $id, $section, $color='#fff', $img='', $imgOrColor = 'img'){
    ...
    $wp_customize->add_control(
        new Ruh_Customize_Heading(
            ...
            'ruh_'.$id.'_title_heading1',
            array(
                'settings'      => 'ruh_'.$id.'_title_heading1',
                'section'       => $section,
                'label'         => __( 'Manage Background', 'Ruh Premium' ),
            )
        )
    );
    $def = ($imgOrColor == 'img')?'off':'on';
    $wp_customize->add_setting(
        'ruh_premium_'.$id.'_section_background',
        array(
            'sanitize_callback' => 'ruh_sanitize_text',
            'default'=> $def,
        )
    ...
    $wp_customize->add_control(
        new Ruh_Switch_Control(
            ...
            'ruh_premium_'.$id.'_section_background',
            array(
                'settings'      => 'ruh_premium_'.$id.'_section_background',
                'section'       => $section,
                'label'         => __( 'Manage Background by', 'Ruh Premium' ),
                'on_off_label'  => array(
                    'on' => __( 'Image', 'Ruh Premium' ),
                    'off' => __( 'Color', 'Ruh Premium' )
                ),
            )
        )
    );
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'ruh_'.$id.'_bg_color',
            array(
                'settings'      => 'ruh_'.$id.'_bg_color',
                'section'       => $section,
                'label'         => __( 'BG Color ', 'Ruh Premium' ),
            )
        )
    );
    // bg image
    ...
    $wp_customize->add_control(
        new WP_Customize_Image_Control(
            ...
            array(
                'section' => $section,
                'settings' => 'ruh_'.$id.'_bg_image',
                'label' => __( 'BG Image ', 'Ruh Premium' ),
                'description' => __('Recommended Image Size: 1900X600px', 'Ruh Premium')
            )
        )
    );
}
...
function lzAddCustomRadio($wpCustomizer, $id, $section, $label="Label"){
    $wpCustomizer->add_setting($id,array('sanitize_callback' => 'ruh_sanitize_text','default' => 'off'));
    $wpCustomizer->add_control(
        new Ruh_Switch_Control($wpCustomizer,$id,
            array(
                'settings'=> $id,
                'section'=> $section,
                'label'=> __( $label, 'Ruh Premium' ),
 ...
 *
 **/
function lzAddElement($wpCustomizer, $id, $section, $type = 'text', $label="Label", $callback ='ruh_sanitize_text', $default='Add description here'){
 ...
 * @param object $wpCustomizer an object of a WP customizer object
 * @param string $id an id for an element and the name that will be used at front end
 * @param string $callback callback function default is "ruh_sanitize_text"
 ...
 *
 **/
function addSetting($wpCustomizer, $id, $callback ='ruh_sanitize_text', $default='Add description here'){
    $params = array('sanitize_callback' => $callback);
    if($default != ""){
        $params['default']= __( $default, 'Ruh Premium' );
    }
...
function lzCustomLable($wpCustomizer, $id, $section, $label="Label"){
    ...
    addSetting($wpCustomizer, $id.'1', 'ruh_sanitize_text', $default='');
    $wpCustomizer->add_control(
        new Ruh_Customize_Heading(
            $wpCustomizer,
            $id.'1',
    ...
    $wpCustomizer->add_control(
        new WP_Customize_Image_Control(
            ...
            array(
                'settings' => $id,
                'section' => $section,
                'description' => __($label, 'Ruh Premium')
            )
        )
    );
}
